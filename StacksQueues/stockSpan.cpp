...
int * stockSpan(int *price, int size)
{
    ...
    stack<int> h;
    //we will push index and not price because
    //15 2 4 6...with price gives 1 1 2 2...but real ans is 1 1 2 3
    // for(int i = 0; i < size; i++)
    // {
    //     if(h.empty() || h.top() > price[i])
    //     {
    //         h.push(i);
    //         ans[i] = 1;
    //     }
    //     else
    //     {
    //         while (!h.empty() && price[h.top()] < price[i])
    //         {
    //             h.pop();
    //         }
    //         if(h.empty())
